...
public class Task<TResult> {
	 ...
	 */
	public static Task<Void> delay(long delay, ScheduledExecutorService executor) {
		return delay(delay, executor, null);
	}
	...
	public static <TResult> Task<Task<TResult>> whenAnyResult(Collection<? extends Task<TResult>> tasks) {
		...
		for (Task<TResult> task : tasks) {
			task.continueWith(new Continuation<TResult, Void>() {
				@Override
				public Void then(Task<TResult> task) throws Exception {
	...
	public static Task<Void> whenAll(Collection<? extends Task<?>> tasks) {
		...
		for (Task<?> task : tasks) {
			final Task<Object> t = (Task<Object>) task;
			//表示在task所在的线程操作.
	 ...
	 */
	public Task<Void> continueWhile(final Callable<Boolean> predicate,
	                                final Continuation<Void, Task<Void>> continuation, final Executor excutor,
	                                final CancellationToken ct) {
		...
		predicateContinuation.set(new Continuation<Void, Task<Void>>() {
			...
			public Task<Void> then(Task<Void> task) throws Exception {
				...
				if (predicate.call()) {
					return Task.<Void>forResult(null).onSuccessTask(continuation, excutor)
							.onSuccessTask(predicateContinuation.get(), excutor);
				}
				return Task.forResult(null);
			}
		});
		return makeVoid().continueWithTask(predicateContinuation.get(), excutor);
	}
	...
	public <TContinuationResult> Task<TContinuationResult> continueWithTask(
			Continuation<TResult, Task<TContinuationResult>> continuation, Executor executor) {
	 ...
	 */
	public <TContinuationResult> Task<TContinuationResult> onSuccess(
			final Continuation<TResult, TContinuationResult> continuation, Executor excutor,
			final CancellationToken ct) {
		return continueWithTask(new Continuation<TResult, Task<TContinuationResult>>() {
			//表示完成了
			@Override
			public Task<TContinuationResult> then(Task<TResult> task) throws Exception {
				if (ct != null && ct.isCancellationRequested()) {
					return Task.cancelled();
				}

				if (task.isFaulted()) {
					return Task.forError(task.getError());//还是会继续执行接下来的任务.
				} else if (task.isCancelled()) {
					return Task.cancelled();//同上
				} else {
					return task.continueWith(continuation);//立即执行接下来的继续任务.
				}
			}
		}, excutor);
	}
				...
				try {
					...
					if (result == null) {
						...
						result.continueWith(new Continuation<TContinuationResult, Void>() {
							@Override
							public Void then(Task<TContinuationResult> task) throws Exception {
								if (ct != null && ct.isCancellationRequested()) {
									tcs.setCancelled();
									return null;
								}
